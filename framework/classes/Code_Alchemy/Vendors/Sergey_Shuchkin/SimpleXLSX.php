...
class SimpleXLSX {
	...
	/**
	 * @var bool true to suppress warnings
	 */
	private $suppress_warnings = false;
	...
	function rows( $worksheet_id = 1 ) {
		...
		if (($ws = $this->worksheet( $worksheet_id)) === false){
            ...
            if ( ! $this->suppress_warnings ) \FB::warn(get_called_class().": Unable to fetch worksheet");
	...
	function rowsEx( $worksheet_id = 1 ) {
		...
		foreach ($ws->sheetData->row as $index => $row) {

			// Hack!  Discard empty rows
			if ( ! $row->c ){
				...
				foreach ($row->c as $c) {
					...
					if ($value && $s > 0 && isset($this->workbook_cell_formats[ $s ])) {
...
//						'name' => chr($i + 65).($curR+1),
	...
	private function _columnIndex( $cell = 'A1' ) {
		...
		if (preg_match("/([A-Z]+)(\d+)/", $cell, $matches)) {
			...
			\FB::error("Invalid cell index.");
	}

	/**
	 * Get the value of a cell in the workbook
	 * @param $cell
	 * @return bool|string
	 */
	function value( $cell ) {
		...
		\FB::info(get_called_class().": Datatype is $dataType");
*/
		switch ($dataType) {
			case "s":
	...
	function _unzip( $filename, $is_data = false ) {
		...
		if ($is_data) {
			...
			if (!is_readable($filename)) {
				...
				\FB::error(get_called_class().": $filename: File not found");
...
//									print_r( $this->workbook_cell_formats );
	...
	/**
	 * Gets rows from Excel file
	 * @param bool $discard_header_row
	 * @return array
	 */
	public function get_rows( $discard_header_row = false ){

		$this->suppress_warnings = true; // suppress warnings

		$rows = array();

		$worksheet_id = 0;

		//\FB::info($this->sheetsCount());

		while ( $worksheet_id <= 10 && ! count( $rows))

			$rows = $this->rows($worksheet_id++);

		return $discard_header_row ? array_splice($rows,1):$rows;
