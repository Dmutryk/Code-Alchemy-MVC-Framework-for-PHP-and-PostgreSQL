...
class UploadHandler {
    ...
    function __construct(){
        ...
        $this->original_filename = $this->getName();
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), array_map("strtolower", $this->allowedExtensions))){
            ...
            $error = "File has an invalid extension $ext, it should be one of:" . $these;

            // sanitize for video
            if ( in_array(strtolower($ext),array('mov','mp4','flv','ogg','ogv','webm','avi')))

                $error = 'Please upload videos through YouTue or Vimeo links.';
        ...
        if ($totalParts > 1){
            ...
            $target = $targetFolder.'/'.$this->new_name();
    ...
    private function new_name(){
        ...
        $name = (string) new Random_Password(20).".". (string)new File_Extension($_FILES[$this->inputName]['name']);

        return $name;
    }

    /**
     * Clean up the original filename so it can be used properly on the server
     * @param $filename
     * @return mixed
     */
    private function cleaned_up_original_name($filename){

        // convert hash into "No-"
        $filename = preg_replace('/\#/','No-',$filename);

        // convert spaces into dashes
        $filename = preg_replace('/\s+/','-',$filename);
        // convert apostrophes and quotes into dashes
