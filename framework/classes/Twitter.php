...
;        // return
     ...
     * @return string
     */
    protected function calculateHeader(array $parameters, $url, $oauth2= false)
    {
        // redefine
        $url = (string) $url;
    ...
    {
        // redefine
        $method2 = preg_match('/oauth2/',$method)?'request':(string) $method;
        ...
        $prefix = preg_match('/oauth2/',$method)?'/oauth2/':'/oauth/';

        // calculate the base string
        $base = $this->calculateBaseString(
            self::SECURE_API_URL . $prefix . $method2, 'POST', $parameters
        );
        ...
        $header = $this->calculateHeader(
            $parameters,
            self::SECURE_API_URL . $prefix . $method2
        );

        // set options
        $options[CURLOPT_URL] = self::SECURE_API_URL . $prefix . $method2;
...
// Timeline resources
     ...
     * parameters.
     * User timelines belonging to protected users may only be requested when the authenticated user either
     * "owns" the timeline or is an approved follower of the owner.
     * The timeline returned is the equivalent of the one seen when you view a user's profile on twitter.com.
     * This method can only return up to 3,200 of a user's most recent Tweets. Native retweets of other statuses
     * by the user is included in this total, regardless of whether include_rts is set to false when requesting
     *
     * this resource.
     *
...
// OAuth resources
     ...
     * @throws \Exception
     */
    public function oAuth2RequestToken($grant_type = 'client_credentials')
    {
        // init var
        $parameters = null;

        $parameters['grant_type'] = (string) $grant_type;

        // make the call
        $response = $this->doOAuthCall('oauth2_token', $parameters);
