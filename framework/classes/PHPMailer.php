...
class PHPMailer {
 ...
 /**
  ...
  public function __construct($exceptions = false) {
      ...
      $configuration = (new \Code_Alchemy\Core\Configuration_File())->find('messaging', true);

      $settings = $configuration->phpmailer;


	$this->Mailer     = $settings?(string)$settings->mailer:"";
	$this->Hostname   = $settings?(string)gethostbyname($settings->hostname):"";
	$this->Host       = $settings?(string)gethostbyname($settings->host):"";
	$this->Port       = $settings?(int)$settings->port:25;
	$this->SMTPSecure = 'no';
	$this->SMTPAuth   = true;
	$this->Username   = $settings?(string)$settings->username:"";
	$this->Password   = $settings?(string)$settings->password:"";
  ...
  private function set_zoho(){

    // For Zoho Mail
    if ( preg_match('/(zoho|gmail|messagingengine)/i',$this->Hostname) ){
  ...
  /**
   * Sets credentials as an override
  ...
  public function set_credentials( \Code_Alchemy\Vendors\PHPMailer\PHPMailer_Credentials $credentials ){
    ...
    $this->Hostname = $credentials->smtp_server;

    $this->Host = $credentials->smtp_server;
 ...
 */
  ...
  protected function SmtpSend($header, $body) {
    ...
    if(!$this->SmtpConnect()) {
        ...
        $error = $this->Lang('smtp_connect_failed');

       // FB::info("Failed to SMTP connect: $error");

        throw new phpmailerException($error, self::STOP_CRITICAL);
    }
    ...
    if(!$this->smtp->Mail($smtp_from)) {
        ...
        //FB::info("From failed for $smtp_from");
  ...
  public function SmtpConnect() {
    ...
    FB::info("Connecting via SMTP",get_called_class());
    ...
    try {
      while($index < count($hosts) && !$connection) {
        ...
        FB::info([$host,$port],get_called_class());
      ...
      //FB::info("successful SMTP connect");
    return true;
  }
