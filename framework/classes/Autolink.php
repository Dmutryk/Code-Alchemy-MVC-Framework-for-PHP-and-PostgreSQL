<?php
namespace Code_Alchemy\Twitter;
...
class Autolink extends Regex {
    ...
    /**
     * @var bool true to send messages to Firebug
     */
   ...
   */
  public function addLinks( $firebug = false ) {

      $this->firebug = $firebug;

    $original = $this->tweet;

      if ( $this->firebug ) \FB::log($original);
  ...
  public function addLinksToURLs() {
      ...
      if ( $this->firebug) \FB::log($this->tweet);
  ...
  protected function _addLinksToURLs($matches) {
      ...
      if ( $this->firebug) \FB::log($matches);

     list($all, $before, $url, $protocol, $domain, $path, $query) = array_pad($matches, 7, '');

      if ( $this->firebug ) \FB::log("All=$all,Before=$before,Url=$url,Protocol=$protocol,Domain=$domain,Path=$path,Query=$query");
