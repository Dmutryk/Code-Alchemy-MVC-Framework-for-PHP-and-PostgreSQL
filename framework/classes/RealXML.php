...
class RealXML {
	...
	public function __construct( $obj_or_src = null ) {
		...
		global $directory_name,$codealchemy_location;

		$codealchemy_location = $codealchemy_location?$codealchemy_location:guessed_location();
        ...
        self::$paths = array(
			...
			"",
			$codealchemy_location . '/xml' ,
			"./xml" ,
			"../xml" 
			);

        if ( $this->debug ) FB::log(new xo_array(self::$paths));
	...
	public function html( 
		...
		global $container,$codealchemy_location,$directory_name;
		$tag = new xo_codetag(xo_basename(__FILE__),__LINE__,get_class(),__FUNCTION__);
