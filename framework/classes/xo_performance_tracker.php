...
class xo_performance_tracker extends magic_object {
    ...
    public function dump_to_FireBug(){
        ob_start();
        $firebug = new FB();
        $firebug->log('BEGIN PERFORMANCE STATISTICS');
        $firebug->log($this->number_sql_queries ." SQL queries performed");
        $firebug->log($this->duplicate_sql_queries . " DUPLICATE SQL queries performed");
        $firebug->log($this->number_unique_sql_queries ." UNIQUE SQL queries performed");
        $firebug->log("Below are the unique queries for the page");
        foreach( $this->unique_sql_queries as $q){
            $firebug->log("$q");
        }
        // sql queries count
        // show unique queries
        $firebug->log("Below are the # of calls for each query");
        foreach( $this->sql_queries_count as $key => $count){
            $firebug->log( $count . " calls to ". $this->unique_sql_queries[$key]);
        }
    }
}
